...
while True:
  ...
  if ret:
    ...
    boxes, player_1_boxes = detection_model.detect_player_1(frame, court_detector)
    cv2.rectangle(frame, (int(player_1_boxes[-1][0]), int(player_1_boxes[-1][1])),
                  (int(player_1_boxes[-1][2]), int(player_1_boxes[-1][3])), [0, 0, 255], 2)
    cv2.imshow('Frame', frame)
    _, tracked_objects = detection_model.detect_top_persons(frame, court_detector, frame_i)
    # ToDo iteration de chaque tracked_persons
    ...
    break
video.release()
print(f'processus in{(time.time() - t0)/60} min ')
print('Finished!')
